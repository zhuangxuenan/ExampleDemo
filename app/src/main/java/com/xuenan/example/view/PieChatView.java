...
public class PieChatView extends View {
    ...
    private void paintPie(final Canvas mCanvas) {
        if (kindsMap != null && centerTitle != null) {
            Set<Map.Entry<String, Double>> entrySet = kindsMap.entrySet();
            ...
            while (iterator.hasNext()) {
                ...
                float needDrawAngle = (float) (num * 1.0f / sum * 360);
                String drawAngle;
                if(needDrawAngle>=18){
                    //drawAngle = dff.format(needDrawAngle / 360 * 100);
                    double num_d = new BigDecimal(num).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                    double sum_d = new BigDecimal(sum).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                    drawAngle = dff.format((num_d / sum_d) * 100);
                    //TODO  kinds = kinds + "," + drawAngle + "%";
                    kinds = drawAngle + "%";
                }else{
                    kinds="";
                }
                ...
                if (Math.min(needDrawAngle, animatedValue - currentAngle) >= 0) {
                    ...
                    AppLogMessageMgr.e(i+"---"+currentAngle+"----"+(animatedValue - currentAngle)+"---"+needDrawAngle);
    ...
    public void setDataMap(LinkedHashMap<String, Double> map) {
        this.kindsMap = map;
        sum = getSum(map);
    }
    ...
    private double getSum(Map<String, Double> map) {
        ...
        while (iterator.hasNext()) {
            ...
            double num = map.get(kinds);
            AppLogMessageMgr.e("--------"+num);
            sum = sum + num;
        }
        return sum;
    }
