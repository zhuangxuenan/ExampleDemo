...
/**glide图片加载工具类
 * Created by zhuangxuenan on 2017/3/6.
 */
...
public class GlideUtils {
    ...
    //加载本地文件drawable mipmap资源
    public static void loadImageView(Context mContext,int path, ImageView mImageView) {
        Glide.with(mContext).load(path).into(mImageView);
    }
    //实现高斯模糊 radius越大 模糊度越高
    /**
     * @param mContext 上下文
     * @param mImageView 控件
     * @param path 资源
     */
    public static void loadImageView(Context mContext,int path, ImageView mImageView,int radius) {
     ...
     * @param path 资源
     */
    public static void loadImageViewCircle(Context mContext,int path, ImageView mImageView) {
        Glide.with(mContext).load(path).bitmapTransform(new CropCircleTransformation(mContext)).into(mImageView);
    }
    //圆角
    /**
     * @param mContext 上下文
     ...
     * @param path 资源
     */
    public static void loadImageViewRound(Context mContext,int path, ImageView mImageView,int radius,int margin) {
        Glide.with(mContext).load(path).bitmapTransform(new RoundedCornersTransformation(mContext,radius,margin, RoundedCornersTransformation.CornerType.ALL)).into(mImageView);
    }
    //灰度处理
    /**
     * @param mContext 上下文
     ...
     * @param path 资源
     */
    public static void loadImageViewGray(Context mContext,int path, ImageView mImageView) {
        Glide.with(mContext).load(path).bitmapTransform(new GrayscaleTransformation(mContext)).into(mImageView);
    }
    //实现高斯模糊 radius越大 模糊度越高
    /**
     * @param mContext 上下文
     ...
     * @param path 资源
     */
    public static void loadImageView(Context mContext,String path, ImageView mImageView,int radius) {
        Glide.with(mContext).load(path).bitmapTransform(new BlurTransformation(mContext,radius)).into(mImageView);
    }
    //圆形
    /**
     ...
     * @param path 资源
     */
    public static void loadImageViewRound(Context mContext,String path, ImageView mImageView,int radius,int margin) {
        Glide.with(mContext).load(path).bitmapTransform(new RoundedCornersTransformation(mContext,radius,margin, RoundedCornersTransformation.CornerType.ALL)).into(mImageView);
    }
    //灰度处理
    /**
     ...
     * @param path 资源
     */
    public static void loadImageViewGray(Context mContext,String path, ImageView mImageView) {
        Glide.with(mContext).load(path).bitmapTransform(new GrayscaleTransformation(mContext)).into(mImageView);
    }
    //加载本地文件drawable mipmap资源 指定大小
    public static void loadImageView(Context mContext,int path, ImageView mImageView, int width, int height) {
        Glide.with(mContext).load(path).override(width, height).into(mImageView);
    }
    //加载本地文件 sd卡 assets raw 资源
    public static void loadImageView(Context mContext,int tag,String path, ImageView mImageView) {
        if(tag==1){//sd卡
            Glide.with(mContext).load("file://"+path).into(mImageView);
        }else if(tag==2){//assets
            Glide.with(mContext).load("file:///"+path).into(mImageView);
        }else if(tag==3){//raw
            Glide.with(mContext).load("Android.resource://com.frank.glide/raw/"+path).into(mImageView);
        }
    }
    //加载指定大小
    ...
    /**
     * api也提供了几个常用的动画：比如crossFade()淡入淡出动画 dontAnimate()无动画  animate(int anim) 自定义
     */
